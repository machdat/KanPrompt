...
`git pull`-Block in `install/update.ps1` eingefügt. Das Script erkennt automatisch ob es in einem Git-Repo mit Remote läuft und führt `git pull` vor dem Copy-Schritt aus. Bei Fehler (kein Netzwerk, kein Git, kein Repo) läuft das Script mit lokalen Dateien weiter.

## Geänderte Dateien

- `install/update.ps1` — Neuer `git pull`-Block zwischen Repo-Root-Check und Versions-Read eingefügt
