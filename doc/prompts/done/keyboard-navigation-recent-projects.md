...
- **Datum:** 2026-03-15T19:35:00
- **Branch:** master
- **Ergebnis:** Erfolgreich

## Zusammenfassung

Keyboard-Navigation für die Recent-Projects-Liste auf dem Welcome-Screen implementiert. Pfeiltasten ↑/↓ navigieren mit Wrap-Around, Enter öffnet das fokussierte Projekt, Escape kehrt zum Board zurück (wenn von dort gekommen). Maus-Hover und Keyboard-Fokus koexistieren. `switchProject()` wurde refactored: State wird erst beim tatsächlichen Projektwechsel zurückgesetzt, nicht beim Anzeigen des Welcome-Screens.

## Geänderte Dateien

- `kanprompt.html` — CSS `.recent-item.focused` Style, `setRecentFocus()`/`getRecentFocusedName()` Fokus-Verwaltung, `switchProject()` refactored (kein State-Reset mehr), `resumeBoard()` und `resetProjectState()` neu, Keyboard-Handler mit Welcome-Screen-Modus (↑/↓/Enter/Escape), Auto-Fokus nach switchProject(), mouseenter-Listener auf Recent-Items
